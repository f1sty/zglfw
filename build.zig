...
fn addIncludePaths(b: *std.Build, unit: anytype, target: std.Build.ResolvedTarget, options: anytype) void {
    switch (@TypeOf(unit)) {
        *std.Build.Step.Compile => unit.root_module.addIncludePath(b.path("libs/glfw/include")),
        *std.Build.Module => unit.addIncludePath(b.path("libs/glfw/include")),
        else => unreachable
    }
    switch (target.result.os.tag) {
        .linux => {
            if (b.lazyDependency("system_sdk", .{})) |system_sdk| {
                switch (@TypeOf(unit)) {
                    *std.Build.Step.Compile => unit.root_module.addSystemIncludePath(system_sdk.path("linux/include")),
                    *std.Build.Module => unit.addSystemIncludePath(system_sdk.path("linux/include")),
                    else => unreachable
                }
                if (options.enable_wayland) {
                    switch (@TypeOf(unit)) {
                        *std.Build.Step.Compile => unit.root_module.addSystemIncludePath(system_sdk.path("linux/include/wayland")),
                        *std.Build.Module => unit.addSystemIncludePath(system_sdk.path("linux/include/wayland")),
                        else => unreachable
                    }
                }
            }
